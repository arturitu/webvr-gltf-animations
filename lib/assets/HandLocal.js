...
function HandLocal (app, isLeftHand) {
  Hand.call(this, app, isLeftHand);
  this.touched;
...
HandLocal.prototype.modelLoaded = function(obj){
  ...
  if(this.isLeftHand){
    this.app.emitter.emit('handLAdded', this.mesh);
  }else{
    this.app.emitter.emit('handRAdded', this.mesh);
  }
}
HandLocal.prototype.update = function (dt, time) {
  ...
  if(this.gamepad && this.mesh){
    ...
    this.position.set(this.pose.position.x,this.pose.position.y-this.app.controls.userHeight,this.pose.position.z);
