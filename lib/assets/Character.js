...
Character.prototype.update = function (delta, time) {
  ...
  breathScale = 1 + (Math.sin(time*0.0015) + 1)/32;
  this.scale.set(breathScale,breathScale,breathScale);
  if(body){
    if(lookVec3.x>0){
      body.morphTargetInfluences[2] = THREE.Math.clamp(lookVec3.x*6,0,1);
    }else{
      body.morphTargetInfluences[3] = -THREE.Math.clamp(lookVec3.x*6,-1,0);
    }
    if(lookVec3.y>0){
      body.morphTargetInfluences[0] = -THREE.Math.clamp(lookVec3.y*6,0,1);
    }else{
      body.morphTargetInfluences[1] = THREE.Math.clamp(lookVec3.y*6,-1,0);
    }
  }
};
