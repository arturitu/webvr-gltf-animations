...
function HUD (app) {
  ...
  this.app.emitter.on('connected',this.connected.bind(this));
  this.app.emitter.on('disconnected',this.disconnected.bind(this));
}
...
HUD.prototype.getRandomRoom = function (){
    ...
    let roomID = "";
		...
		roomID += possibleLetters.charAt(Math.floor(Math.random() * possibleLetters.length));
		roomID += possibleNumbers.charAt(Math.floor(Math.random() * possibleNumbers.length));
    // console.log('Comment on PRODUCTION');
    // roomID = 't3s1';
...
HUD.prototype.checkURL = function (){
  ...
  if ( index >= 0 ) {
    ...
    if(isValidID(id)){
      ...
      document.querySelector('#room').style.display = 'none';
      document.querySelector('#alert').style.display = 'block';
      document.querySelector('#alert').innerHTML = 'This Room ID does not exist.';
      return 'error';
    }
  }else{
    document.querySelector('h3').style.display = 'block';
